<template>
  <div class="main user-layout-register">
    <h3 style="text-align: center;"><span>注册</span></h3>
...
<script>
// getCAPTCHAP, register
import { mixinDevice } from '@/utils/mixin.js'
import { getCAPTCHAP } from '@/api/login'
      ...
      validateFields(['mobile'], { force: true },
        (err, values) => {
          if (!err) {
            ...
            getCAPTCHAP({ phone: values.mobile, operationType: 0 }).then(res => {
              setTimeout(hide, 2500)
              $notification['success']({
                message: '提示',
                description: '验证码获取成功，您的验证码为：' + res.message,
                duration: 8
              })
            }).catch(err => {
              console.log(err)
