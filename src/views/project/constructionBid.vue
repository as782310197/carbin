<template>
  <a-card class="constructionBidCard">
    <div>
      <!-- 搜索部分 -->
      <div class="searchDiv">
        <a-form class="ant-advanced-search-form" :form="searchForm" :label-col="{ span: 8 }" :wrapper-col="{ span: 14 }" @submit="handleSearch">
          <a-row :gutter="24">
            <a-col :span="8">
              <a-form-item label="所属监理标段">
                <a-select
                  v-decorator="['supervisionSection']"
                  placeholder="请选择所属监理标段"
                >
            ...
            <a-col :span="8">
              <a-form-item label="施工标段名称">
                <a-input
                  v-decorator="['sectionName']"
                  placeholder="请输入施工标段名称"
                />
              </a-form-item>
            </a-col>
      ...
      >
        <a-form :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }" @submit="handleSubmit">
          ...
          <a-form-item label="所属监理标段">
            <a-select
              v-decorator="['supervisionSection', {
                rules: [{ required: true, message: '请选择所属监理标段' }]
              }]"
              placeholder="请选择所属监理标段"
            >
              <a-select-option v-for="item in supervisionBidDatas" :key="item.id" :value="item.id">{{ item.sectionName }}</a-select-option>
          ...
          <a-form-item label="施工标段编号">
            <a-input
              v-decorator="['sectionNo', {
                rules: [{ required: true, message: '施工标段编号不能为空' }]
              }]"
              placeholder="请输入施工标段编号"
            />
          </a-form-item>
          <a-form-item label="施工标段名称">
            <a-input
              v-decorator="['sectionName', {
                rules: [{ required: true, message: '施工标段名称不能为空' }]
              }]"
              placeholder="请输入施工标段名称"
            />
          </a-form-item>
      ...
      >
        <a-tabs>
          <a-tab-pane key="1" tab="基本信息">
            <detail-list :col="2">
              <detail-list-item term="所属监理标段">{{ obj.supervisionName }}</detail-list-item>
              <detail-list-item term="施工标段编号">{{ obj.sectionNo }}</detail-list-item>
            </detail-list>
            <detail-list :col="2">
              <detail-list-item term="施工标段名称">{{ obj.sectionName }}</detail-list-item>
              <detail-list-item term="创建时间">{{ obj.createTime }}</detail-list-item>
            </detail-list>
            <detail-list :col="1">
              <detail-list-item term="单位">{{ obj.enterpriseName }}</detail-list-item>
            </detail-list>
          </a-tab-pane>
          <a-tab-pane key="2" tab="工点">
            <s-table
              size="small"
              :columns="workPointColumns"
              :data="loadWorkPointData"
        ...
        >
          <detail-list :col="2">
            <detail-list-item term="所属监理标段">白云山隧道</detail-list-item>
            <detail-list-item term="施工标段编号">1234242</detail-list-item>
          </detail-list>
          <detail-list :col="2">
            <detail-list-item term="施工标段名称">xxxxxx</detail-list-item>
            <detail-list-item term="创建时间">2020.03.28 18:25:24</detail-list-item>
          </detail-list>
          <detail-list :col="1">
            <detail-list-item term="单位信息">xxx单位</detail-list-item>
          </detail-list>
        </a-card> -->
      </a-drawer>
    </div>
...
import { getConstructionBidPage, getConstructionBidInfo, saveConstructionBid, removeConstructionBid } from '@/api/constructionBid'
import { getWorksitePage } from '@/api/worksite'
...
export default {
  ...
  data () {
    return {
      ...
      loadWorkPointData: parameter => {
        var param = {
          entity: {
            constructionSection: this.worksiteParam
          }
        }
  ...
  methods: {
    ...
    handleSearch (e) {
      ...
      this.searchForm.validateFields((error, values) => {
        if (!error) {
          ...
          var param = {
            entity: {
              supervisionSection: values.supervisionSection,
    ...
    handleEdit (e) {
      ...
      this.visible = true
      const param = { id: e.id }
      getConstructionBidInfo(param).then(res => {
        this.obj = res.result
        this.form.setFieldsValue(pick(res.result, 'id', 'supervisionSection', 'sectionNo', 'sectionName', 'enterpriseId'))
      })
    },
    // 保存按钮
    handleSubmit (e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          saveConstructionBid(values).then(res => {
            if (res.code === 1) {
              // this.$success({
              //   title: '提示信息',
              //   content: '保存成功',
    ...
    handleRead (e) {
      // console.log('查看数据！')
      this.readTitle = '查看施工标信息'
      this.visibleRead = true
      var param = { id: e.id }
      this.worksiteParam = e.id
      getConstructionBidInfo(param).then(res => {
        this.obj = res.result
      })
    ...
    confirm (e) {
      ...
      removeConstructionBid(param).then(res => {
        if (res.code === 1) {
          this.$success({
            title: '提示信息',
            content: '删除成功',
